using BooksNET8API.Controllers;
...
namespace BookRestAPIUnitTest
{
    public class BookRestAPIUnitTest
    {
        ...
        public async Task GetBook_BasedOnId_WithNotExistingBook_ReturnNotFound()
        {
            ...
            var model = Assert.IsAssignableFrom<ICollection<Book>>(okResult);
            Assert.Equivalent(sampleBook, model);

            //Assert.Equal("expected result", sampleBook.Name);
            //Assert
        ...
        public async Task GetBook_BasedOnId_WithExistingBook_ReturnBook()
        {
            ...
            booksController = new BooksController(bookStub.Object);
            //Act
            var actionResult = await booksController.GetBook(1234);
            //Assert
